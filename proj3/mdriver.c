...
static int eval_mm_check(malloc_impl_t *impl, trace_t *trace, int tracenum);

/* Various helper routines */
static void printresults(int n, char **tracefiles, stats_t *stats);
static void usage(void);

/* Struct of function pointers for the mm malloc implementation. */
static malloc_impl_t mm_impl = {
  &mm_init,
  &mm_malloc,
  &mm_realloc,
  &mm_free,
...
/* Libc can't be reset. */
static void libc_reset(void)
{
}

/* Return NULL for the minimum pointer value.
 */
static void *libc_heap_lo(void)
{
  return NULL;
}

/* Return NULL - 1 as a way to compute the maximum pointer value.  This
 * probably isn't portable.
 */
static void *libc_heap_hi(void)
{
  return NULL - 1;
}

/* Struct of function pointers for the libc malloc implementation. */
static malloc_impl_t libc_impl = {
  &libc_init,
  &malloc,
  &realloc,
  &free,
...
/* Struct of function pointers for a buggy malloc implementation. */
static malloc_impl_t bad_impl = {
  &bad_init,
  &bad_malloc,
  &bad_realloc,
  &bad_free,
...
int main(int argc, char **argv)
{
  ...
  for (i = 0; i < num_tracefiles; i++) {
    ...
    mm_stats[i].valid = eval_mm_valid(&mm_impl, trace, i);
    if (check_heap) {
      mm_stats[i].checked = eval_mm_check(&mm_impl, trace, i);
    }
    if (mm_stats[i].valid) {
      if (verbose > 1)
        printf("efficiency, ");
  ...
  for (i = 0; i < trace->num_ops; i++) {
    switch (trace->ops[i].type) {
      ...
      case REALLOC: /* realloc */
        ...
        newsize = trace->ops[i].size;
        oldp = trace->blocks[index];
        if ((newp = impl->realloc(oldp,newsize)) == NULL) {
